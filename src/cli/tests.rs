...
fn raw_opt_no_options() {
    ...
    assert_eq!(raw_opt(&["lock"]), RawOpt {
        ...
        config_file:     None,
        profile:         None,
        command:         RawCommand::Lock { update: false, reinstall: false },
    });
}
...
fn raw_opt_add_git_with_options() {
    ...
    assert_eq!(
        ...
        RawCommand::Add(Box::new(Add {
            name:     "test".to_string(),
            git:      Some("https://github.com/rossmacarthur/sheldon-test".parse().unwrap()),
            gist:     None,
            github:   None,
            remote:   None,
            local:    None,
...
fn raw_opt_add_local_with_reference_expect_conflict() {
    setup();
    assert_eq!(
        raw_opt_err(&["add", "test", "--local", "~/.dotfiles/zsh/pure", "--tag", "0.1.0"]).kind(),
        ErrorKind::ArgumentConflict
    );
}
...
fn raw_opt_add_local_with_protocol_expect_conflict() {
    setup();
    assert_eq!(
        raw_opt_err(&["add", "test", "--local", "~/.dotfiles/zsh/pure", "--proto", "ssh",]).kind(),
        ErrorKind::ArgumentConflict
    );
}
